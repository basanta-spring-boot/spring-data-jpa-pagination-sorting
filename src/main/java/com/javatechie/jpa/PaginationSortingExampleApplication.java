...
public class PaginationSortingExampleApplication {
    ...
    @GetMapping("/{field}")
    private APIResponse<List<Product>> getProductsOrdersByField(@PathVariable String field) {
        List<Product> allProducts = service.findAllProductsOrderBy(field);
        return new APIResponse<>(allProducts.size(), allProducts);
    }

    @GetMapping("/pagination/{offset}/{pageSize}")
    private APIResponse<Page<Product>> getProductsWithPagination(@PathVariable int offset, @PathVariable int pageSize) {
        Page<Product> allProducts = service.findAllProductsWithPagination(offset, pageSize);
    ...
    private APIResponse<Page<Product>> getProductsWithPaginationWithSort(@PathVariable int offset, @PathVariable int pageSize,@PathVariable String field) {
        Page<Product> allProducts = service.findAllProductsWithPaginationWithSort(offset, pageSize,field);
        return new APIResponse<>(allProducts.getSize(), allProducts);
    }
